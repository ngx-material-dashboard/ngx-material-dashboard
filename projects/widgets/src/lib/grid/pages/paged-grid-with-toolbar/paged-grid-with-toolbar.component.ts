...
export class PagedGridWithToolbarComponent<T extends JsonModel> implements AfterContentInit, OnDestroy, OnInit {
    ...
    @ContentChild(PagedGridComponent) grid!: PagedGridComponent<T>;
    ...
    ngAfterContentInit(): void {
        ...
        this.filter.searchClick.subscribe(() => {
            ...
            this.grid.dataSource$.filter = this.buildSearchFilter(searchFilterData);

            if (this.grid.dataSource$ instanceof RemoteDataSource) {
     ...
     */
     onToolbarButtonClick(buttonClick: ButtonClick): void {
        if (!this.grid.selection.isEmpty()) {
            ...
            buttonClick.row = this.grid.selection.selected[0];
        }
        this.buttonClick.emit(buttonClick);
    }
}
