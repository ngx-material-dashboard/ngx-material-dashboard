        ...
        <div class="marker-pagedTable">
            <mat-table [dataSource]="dataSource">
                ...
                <ng-container matColumnDef="actions">
                    ...
                    <mat-cell
                        class="actions-cell mat-row-buttons"
                        *matCellDef="let row"
                    >
                        <button
...
describe('PagedTableElement', () => {
    ...
    describe('Table With Data', () => {
        ...
        beforeEach(() => {
            ...
            actionButtonSpy = spyOn(
                pagedTableElement.component,
                'onActionButtonClick'
            );
        });
        ...
        it('should return "1 – 25 of 200" for paginator range when length=200 and pageSize=25', () => {
            expect(
                pagedTableElement.paginator.pagingatorRange.innerText
            ).toEqual('1 – 25 of 200');
        });

        it('should return "No data found" for noData row', () => {
            expect(pagedTableElement.noDataRow.innerText.trim()).toEqual(
                'No data found'
            );
        });
        ...
        it('should throw an error when trying click button that does not exist in first row', () => {
            expect(() => {
                pagedTableElement.clickItemButton('undefined', 0);
            }).toThrowError(
                'Expected HTMLButtonElement with CSS selector ".marker-button-undefined" in collection item buttons'
            );
        });
    });
    ...
    describe('Table Without Data', () => {
        ...
        it('should return "No data found" for noData row', () => {
            expect(pagedTableElement.noDataRow.innerText).toEqual(
                'No data found'
            );
        });
        ...
        it('should throw an error when something other than mat-row not found in occurs when querying for mat-rows', () => {
            ...
            expect(() => {
                pagedTableElement.data = getTaskData();
            }).toThrowError('I am a random error');
        });
    });
});
